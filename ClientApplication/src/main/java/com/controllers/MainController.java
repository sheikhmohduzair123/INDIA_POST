...
public class MainController {
	...
	/*
	 * @Autowired private SUserService userService;
	 */
	@Autowired
	...
	/*
	 * @Autowired private SUserRepository sUserRepository;
	 *
	...
	public String loginPage(Model model,HttpServletRequest req,HttpServletResponse res) throws Exception {
		...
		if((!userUpdateService.getUserDetailsLocal() || !clientService.getClientDetailsLocal()) && (activeProfiles[0].equals("client")||(activeProfiles[0].equals("clientonly"))))
		{
				...
				!(SecurityContextHolder.getContext().getAuthentication() instanceof AnonymousAuthenticationToken))
		{
			...
			if(loginedUser.isFirstLogin() && !(loginedUser.getRole().getName().equalsIgnoreCase("ROLE_ADMIN")))
			{
				...
				if(activeProfile.equals("client"))
				{
					...
					ratetokenUtils.JwtAuthenticate(client, res);
					...
					/*
					 * Cookie cookie[] = req.getCookies(); for (Cookie c : cookie) { key =
					 * c.getName(); keyToken = c.getValue();
					 * System.out.println("c ::: name"+c.getName() +"value ::"+c.getValue()); if
					 * (c.getName().equals("server")) {
					 * log.debug("server token found::"+c.getValue()); server_flag=1; } else {
					 * System.out.println("cookie not found"); } }
					 * System.out.println("server_flag"+server_flag);
					 *
					 * if(server_flag==0) { System.out.println("1");
					 * log.debug("server token regenerating"); //generate jwt token for server
			...
			/*
			 * if(activeProfile.equals("client")) { int server_flag=0;
			 *
			 * //getting token from cookie Cookie cookie[] = req.getCookies(); for (Cookie c
			 * : cookie) { if (c.getName().equals("server")) { server_flag=1; } }
			 * List<Client> clientList =
			 * clientRepository.findByClientStatusAndStatus("approved",Enum.valueOf(Status.
			 * class, "ACTIVE")); Client client = clientList.get(0);
			 *
			 * if(server_flag==0) { System.out.println("3");
			 *
			 * log.debug("server token regenerating"); //generate jwt token for server
			 * //JwtServerAuthenticate(client,res); } }
			 */
			if((activeProfile.equals("server"))&&((loginedUser.getRole().getName().equals("ROLE_RMS_USER")) || (loginedUser.getRole().getName().equals("ROLE_PO_USER"))))
			{
		    	log.info("\nLogin as UserName:: "+loginedUser.getName() +" with Role:: "+ loginedUser.getRole().getName());
	...
	public String changePasswordHome(Principal principal) {
        ...
        //if user has already set the password, redirect to senderHome page(default client parcel booking page)
	...
	/*
	 * private void JwtAuthenticate(Client client,HttpServletResponse res) throws
	 * Exception {
	 *
	 * RestTemplate restTemplate=restTemplate(); String jwtToken =
	 * restTemplate.postForObject(rateURL +"/parcel/authenticate", client,
	 * String.class); ObjectMapper objectMapper = new ObjectMapper();
	 * JwtTokenResponse jwtTokenResponse = objectMapper.readValue(jwtToken,
	 * JwtTokenResponse.class); jwtToken = jwtTokenResponse.getToken();
	 ...
	 *
	 *
	 * Cookie cookie=writeCookie(jwtToken); res.addCookie(cookie);
	 *
	 * return jwtToken; }
	 */
	 ...
	 * //cookie.setSecure(true); // cookie.setDomain(baseurl); //
	 * cookie.setPath("/"); cookie.setHttpOnly(true); return cookie;
	 *
	 * }
	 */
	...
	/*
	 * private void JwtServerAuthenticate(Client client,HttpServletResponse res)
	 * throws Exception { log.info(" getting token on rest call "+client);
	 * RestTemplate restTemplate = restTemplate(); try { String jwtToken =
	 * restTemplate.postForObject(serverURL + "/server/authenticate", client,
	 * String.class); ObjectMapper objectMapper = new ObjectMapper();
	 * JwtTokenResponse jwtTokenResponse = objectMapper.readValue(jwtToken,
	 * JwtTokenResponse.class); jwtToken = jwtTokenResponse.getToken(); Cookie
	 * cookie=writeServerCookie(jwtToken); res.addCookie(cookie); } catch (Exception
	 * e){ e.printStackTrace(); } }
	 */
	 ...
	 *
	 * log.info("writing cookieToken to browser "); Cookie cookie = new
	 * Cookie("server",token); //cookie.setMaxAge(365 * 24 * 60 * 60); // expires in
	 * for one year cookie.setMaxAge(Integer.parseInt(cookieMaxAge)); // for one day
	 * //cookie.setSecure(true); cookie.setHttpOnly(true);
	 * //cookie.setDomain(baseurl); //cookie.setPath("/"); return cookie; }
	 */
