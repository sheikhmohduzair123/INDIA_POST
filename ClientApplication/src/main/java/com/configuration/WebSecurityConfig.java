...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter{
	 ...
	 */
		...
		public void configure(HttpSecurity http) throws Exception {
			...
			http.authorizeRequests().antMatchers("/", "/login", "/logoutSuccessful","/user/activateUser","/pbTracking/trackParcel","/pbTracking/getDetailsWithParcelId").permitAll();
		...
		//	http.authorizeRequests().antMatchers("/admin").access("hasRole('ROLE_ADMIN')");
			...
			http.authorizeRequests().antMatchers("/pbTracking/*").access("hasAnyRole('ROLE_ADMIN','ROLE_PO_USER','ROLE_RMS_USER')");
			http.authorizeRequests().antMatchers("/masterController/*").access("hasRole('ROLE_ADMIN')");
			// http.authorizeRequests().antMatchers("/client/*").access("hasRole('ROLE_FRONT_DESK_USER')");
			...
			http.authorizeRequests().and().formLogin()
					...
					.and().logout()
					.logoutRequestMatcher(new AntPathRequestMatcher("/logoutSuccessful"))
			...
			http.headers()
		       .addHeaderWriter(new StaticHeadersWriter("X-Content-Security-Policy","script-src 'self' 'unsafe-inline' 'unsafe-eval'; font-src 'self'; default-src 'self' 'unsafe-inline'; connect-src 'self'; img-src 'self'; style-src 'self' 'unsafe-inline'; object-src 'self'; media-src 'self'; data: 'unsafe-inline'; "));
