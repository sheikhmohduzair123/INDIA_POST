...
public class ForgotPasswordTest {
	...
	public void resetPassword() throws Exception {
		...
		assertEquals(updatePasswordError.getPasswordresetsuccessful(),
				userDetailService.resetPassword(user, newPassword ));
	}

	@Test
	public void notSameAsOld() throws Exception {
		...
		passwordList.add(new PasswordHistory(user, previousPassword));
		Mockito.when(passwordRepository.findTop5ByUserIdOrderByCreatedOn(1)).thenReturn(passwordList);
		Mockito.when(updatePasswordError.getPasswordresetsuccessful())
				.thenReturn("Password Reset Successful.");
		assertEquals(updatePasswordError.getPasswordresetsuccessful(), userDetailService.resetPassword(user, newPassword));
	}

	@Test
	public void notSameAsPrevious() throws Exception {
