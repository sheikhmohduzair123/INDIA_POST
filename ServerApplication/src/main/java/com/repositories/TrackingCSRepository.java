...
public interface TrackingCSRepository extends JpaRepository<TrackingCS, Long> {
		...
		List<TrackingCS> findTrackingCSByStatusAndUpdatedByInAndBagIdIsNull(Status active, List<User> users);
		
		List<TrackingCS> findBybagFillStatus(BagFillStatus string);
		List<TrackingCS> findBybagFillStatusAndBagStatusInAndUpdatedByInAndStatus(BagFillStatus string,List<BagStatus> baList,List<User> users,Status status);
		List<TrackingCS> findBybagFillStatusAndBagStatusAndUpdatedByInAndStatus(BagFillStatus empty, BagStatus disposed,
				List<User> users,Status active);
