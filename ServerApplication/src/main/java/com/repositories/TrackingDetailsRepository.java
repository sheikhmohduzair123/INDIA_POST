...
public interface TrackingDetailsRepository extends JpaRepository<TrackingDetails, Long> {
	...
	List<TrackingDetails> findAllBypStatusAndUpdatedByInAndObjParcelServiceInAndUpdatedOnBetweenAndObjParcelStatus(
			PStatus status, List<User> users, List<Long> services, Timestamp fromDate, Timestamp toDate,
			String parcelStatus);

	TrackingDetails findTopByObjParcelOrderByUpdatedOn(Parcel parcel);
